...
# 读取训练数据的Notes
def get_notes():
    filepath = 'C:/Users/10341/Desktop/music/'
    files = os.listdir(filepath)
    Notes = []
    for file in files:
        try:
            print(file)
            stream = converter.parse(filepath + file)  ## 读取 MIDI 文件, 输出 Stream 流类型
            instru = instrument.partitionByInstrument(stream)   ## 获得所有乐器部分
...
def get_model(inputs, notes_len, weights_file=None):
    ...
    model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(256, return_sequences=True), input_shape=(inputs.shape[1], inputs.shape[2])))
    model.add(tf.keras.layers.Dropout(0.4))  # 丢弃 30% 神经元，防止过拟合
    
    model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(256, return_sequences=True)))
    model.add(tf.keras.layers.Dropout(0.4))


    model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(256)))  # return_sequences 是默认的 False，只返回输出序列的最后一个
...
def train():
    ...
    history = model.fit(normal_network_input, network_output, epochs=50, batch_size=128,callbacks=callbacks_list)  # 整体迭代100次，每小批128个
...
def generate_notes(model, network_input, note_name, notes_len):
    ...
    for note_index in range(500):
        ...
        #     item = item.tolist()
        prediction_input = prediction_input.astype(np.float_)
...
def create_music():
    ...
    # 写入 MIDI 文件
    midi_stream.write('midi', fp='output2.mid')
